    ...
    ui(new Ui::SerialWidget)
{
    ...
    ui->tEdit_shell->insertPlainText("Bottod Uart Shell V0.1\n");
...
//    ui->tEdit_shell->setTextBackgroundColor("black");
    ...
    ui->textEdit->setText("<font color='red' size=20><b>解决Linux中普通用户无权限访问串口设备问题</b></font>");
    ui->textEdit->append("\n\n以root身份执行:\n"
                         "vim /etc/udev/rules.d/70-ttyusb.rules\n\n"
                         "内容为：\n"
                         R"(KERNEL=="ttyUSB[0-9]*",MODE="0666")");
}
...
bool  SerialWidget:: eventFilter(QObject* target,QEvent * event)
{
...
void SerialWidget::sendText(QString & text)
{
//    int indx;
...
void SerialWidget:: scanport(void)
{
    /* 新建串口类 */
...
void SerialWidget::on_tEdit_shell_cursorPositionChanged()
{
    //do nothing;
    if(ui->tEdit_shell->textCursor().position()<minTextCurse)
...
void SerialWidget::on_pButton_send_clicked()
{
    QTextCursor txtcur = ui->tEdit_shell->textCursor();
    txtcur.movePosition(QTextCursor::End,QTextCursor::MoveAnchor);
...
void SerialWidget::on_pushButton_2_clicked()
{
    openprot();
}

void SerialWidget::receiveInfo()
{
    static int flag = 0;
...
void SerialWidget::openprot(const QString & portname,const int baud)
{
//    if(activePort == nullptr)
//        scanport();
...
void SerialWidget::openprot()
{

    SerialWidget::openprot(ui->cBox_Portlist->currentText(),ui->cBox_Baudlist->currentText().toInt());
}
...
void SerialWidget::on_tEdit_shell_textChanged()
{
    QScrollBar *qbar =  ui->tEdit_shell->verticalScrollBar();
    qbar->setValue(qbar->maximum());
}

const QString & SerialWidget::findQuickCompletString(const QString & str)
{
    int retindex = -1;
...
void SerialWidget::on_timerOver(void)
{
    qout<<"timerOver"<<endl;
    if(receiverBuff[0] != '\0')
    {
...
void SerialWidget::on_cBox_ColorList_currentTextChanged(const QString &arg1)
{
...
void SerialWidget::on_pButton_AddquickCompelat_clicked()
{
...
void SerialWidget::on_serialErrorOccured()
{
...
void SerialWidget::on_SerialWidget_destroyed()
{
    qout<<"关闭"<<endl;
}

void SerialWidget::saveseting(void)
{
    QSettings setting("BottodSerialShell","BottodSerialShell");
...
void SerialWidget::loadseting(void)
{
    QSettings setting("BottodSerialShell","BottodSerialShell");
...
void SerialWidget::on_tabWidget_currentChanged(int index)
{
    qout<<"index"<<index;
    if(index == 1)
    {
...
void SerialWidget::on_pButton_delquickCompelat_clicked()
{
...
void SerialWidget::on_checkBox_caseSensitive_stateChanged(int arg1)
{
    if(arg1== 0)
        caseSensitive = Qt::CaseInsensitive;
    else {
